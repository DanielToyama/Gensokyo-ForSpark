...
type Settings struct {
	...
	AtMarkdown             bool   `yaml:"at_markdown"`              // [新增] 真实at(markdown)开关(默认开启): 官方文本链<qqbot-at-user id="openid"/>纯文本实测不渲染, 频道模板语法<at id="openid">群聊实测也不渲染; 开启后含at的群纯文本消息升级为markdown消息(msg_type=2), at以文档"最新格式"<qqbot-at-user id="openid"/>注入, 实测markdown内可渲染真at; fakeReply伪回复前缀的"@昵称"同步升级为真实at; false=维持@昵称文本
