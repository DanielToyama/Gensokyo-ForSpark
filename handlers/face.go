package handlers

// Modified by DanielToyama on 2026-08-31 (Gensokyo-ForSpark fork)
//
// face 段出站转换 —— 终稿。
//
// 官方 bot 能力表(2026-08-31, https://bot.q.qq.com/wiki/develop/api-v2/):
// 消息类型只有 文本 / Markdown / 图片 / 视频 / 语音 / 文件 / 结构化卡片 / Embed /
// 表情表态 / 引用消息 —— **没有"表情"作为消息内容**这一项(表情表态=给消息点赞,
// 不是发送表情)。因此 onebot v11 的 face 段(数字表情)在官方 bot 通道**从根上
// 不存在**:
//   - <emoji:ID> 纯文本通道实测显示原文;
//   - <emoji:ID> markdown 通道实测同样不渲染;
//   - Unicode emoji 字符方案实测行不通。
//
// 三条路全部证伪后, face 段降级为可读占位 "[表情名]"(如 [大哭]); 未收录 id 用
// "[表情:<id>]" 兜底。不丢内容、不中断后续段、不触发 markdown 升级。
//
// 若未来 QQ 开放平台支持表情消息, 只需改造 faceIDToEmoji: 把名字替换成官方支持
// 的真实表情格式即可, faceNameMap 对应表继续复用。

func faceIDToEmoji(id string) string {
	if id == "" {
		return ""
	}
	if name, ok := faceNameMap[id]; ok {
		return "[" + name + "]"
	}
	return "[表情:" + id + "]"
}

// faceNameMap QQ 经典系统表情 id -> 中文名(降级占位用)
var faceNameMap = map[string]string{
	"1":   "微笑",
	"2":   "撇嘴",
	"3":   "色",
	"4":   "发呆",
	"5":   "得意",
	"6":   "流泪",
	"7":   "害羞",
	"8":   "闭嘴",
	"9":   "睡",
	"10":  "大哭",
	"11":  "尴尬",
	"12":  "发怒",
	"13":  "调皮",
	"14":  "呲牙",
	"15":  "惊讶",
	"16":  "难过",
	"17":  "酷",
	"18":  "冷汗",
	"19":  "抓狂",
	"20":  "吐",
	"21":  "偷笑",
	"22":  "愉快",
	"23":  "白眼",
	"24":  "傲慢",
	"25":  "困",
	"26":  "惊恐",
	"27":  "流汗",
	"28":  "憨笑",
	"29":  "悠闲",
	"30":  "奋斗",
	"31":  "咒骂",
	"32":  "疑问",
	"33":  "嘘",
	"34":  "晕",
	"35":  "狂躁",
	"36":  "哀",
	"37":  "鬼脸",
	"38":  "抠鼻",
	"39":  "鼓掌",
	"40":  "坏笑",
	"41":  "左哼哼",
	"42":  "右哼哼",
	"43":  "哈欠",
	"44":  "鄙视",
	"45":  "委屈",
	"46":  "快哭了",
	"47":  "阴险",
	"48":  "亲亲",
	"49":  "吓",
	"50":  "可怜",
	"51":  "菜刀",
	"52":  "西瓜",
	"53":  "啤酒",
	"54":  "篮球",
	"55":  "乒乓",
	"56":  "咖啡",
	"57":  "饭",
	"58":  "猪头",
	"59":  "玫瑰",
	"60":  "凋谢",
	"61":  "嘴唇",
	"62":  "爱心",
	"63":  "心碎",
	"64":  "蛋糕",
	"65":  "闪电",
	"66":  "炸弹",
	"67":  "刀",
	"68":  "足球",
	"69":  "虫子",
	"70":  "便便",
	"71":  "月亮",
	"72":  "太阳",
	"73":  "礼物",
	"74":  "拥抱",
	"75":  "强",
	"76":  "弱",
	"77":  "握手",
	"78":  "胜利",
	"79":  "抱拳",
	"80":  "勾引",
	"81":  "拳头",
	"82":  "差劲",
	"83":  "爱你",
	"84":  "NO",
	"85":  "OK",
	"86":  "爱情",
	"87":  "飞吻",
	"88":  "跳跳",
	"89":  "发抖",
	"90":  "怄火",
	"91":  "转圈",
	"92":  "磕头",
	"93":  "回头",
	"94":  "跳绳",
	"95":  "投降",
	"96":  "激动",
	"97":  "乱舞",
	"98":  "献吻",
	"99":  "左太极",
	"100": "右太极",
	"101": "邪恶笑",
	"102": "笑眼",
	"103": "花痴",
	"104": "小眼神",
	"105": "红心",
	"106": "怄火",
}
